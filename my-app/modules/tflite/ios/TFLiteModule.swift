...
public class TFLiteModule: Module {
  ...
  // Dedicated serial queue for inference (prevents concurrent inferences)
  private let inferenceQueue = DispatchQueue(label: "com.vespvision.tflite.inference", qos: .userInitiated)
  ...
  public func definition() -> ModuleDefinition {
    ...
    AsyncFunction("runInference") { (inputArray: [Double], inputShape: [Int], promise: Promise) in
      // Use dedicated inference queue (serial - processes one at a time)
      // This prevents multiple concurrent inferences and ensures smooth processing
      self.inferenceQueue.async {
        ...
        do {
          // Optimized: Use UnsafeMutablePointer for faster conversion (saves ~50-100ms)
          let bufferSize = inputShape.reduce(1, *)
          let floatData = UnsafeMutablePointer<Float>.allocate(capacity: bufferSize)
          defer { floatData.deallocate() }
          
          // Fast conversion: direct memory access
          for i in 0..<bufferSize {
            floatData[i] = Float(inputArray[i])
          }
          ...
          // Optimized: Direct conversion to Double array (saves ~20-50ms)
          let result = UnsafeMutablePointer<Double>.allocate(capacity: outputCount)
          defer { result.deallocate() }
          
          outputData.withUnsafeBytes { (bytes: UnsafeRawBufferPointer) in
            let floatPointer = bytes.bindMemory(to: Float.self)
            for i in 0..<outputCount {
              result[i] = Double(floatPointer[i])
            }
          }
          
          let resultArray = Array(UnsafeBufferPointer(start: result, count: outputCount))
